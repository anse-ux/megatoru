...
const makeEventBuffer = (logger) => {
    ...
    // Increase max listeners to avoid warnings with many concurrent operations
    ...
    let bufferCount = 0
    const MAX_HISTORY_CACHE_SIZE = 5000
    const BUFFER_TIMEOUT_MS = 2000
    const MICRO_FLUSH_DELAY = 5
    
    
    let microFlushTimer = null
    ...
    function scheduleMicroFlush() {
        if (microFlushTimer) return
        microFlushTimer = setTimeout(() => {
    ...
    return {
        ...
        emit(event, evData) {
            if (isBuffering && BUFFERABLE_EVENT_SET.has(event)) {
                ...
                // Schedule micro-flush for responsiveness
                scheduleMicroFlush()
                return true
            }
