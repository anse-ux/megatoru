...
const getImageProcessingLibrary = exports.getImageProcessingLibrary = async () => {
...
async function generateThumbnail(file, mediaType, options) {
    ...
    if (mediaType === 'image') {
        ...
        try {
            const buff = await extractVideoThumb(file, '00:00:00', { width: 320 })
...
const encryptedStream = async (media, mediaType, { logger, saveOriginalFileIfRequired, opts, shouldConvert, mediaKey: providedMediaKey } = {}) => {
    let { stream, type } = await getStream(media, opts)
    
    if (shouldConvert && mediaType === 'audio') {
